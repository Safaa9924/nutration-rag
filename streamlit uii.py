...
from llm_client import call_openrouter, LLMClientError, COMMON_MODELS
...
if run:
    ...
    st.session_state.pop("answer", None)  # clear stale answer from a previous question
...
if "context_result" in st.session_state:
    ...
    prompt_package = st.session_state["prompt_package"]

    tab_plan, tab_retrieved, tab_reranked, tab_context, tab_prompt, tab_answer, tab_diag = st.tabs(
        ["🧩 Query Plan", "📥 Retrieved", "🎯 Reranked", "📄 Final Context",
         "🧠 Prompt", "💬 Answer", "🧪 Diagnostics"]
    )
    ...
    with tab_prompt:
        ...
        st.text_area("system_prompt", prompt_package.system_prompt, height=180, label_visibility="collapsed")

        st.subheader("User prompt (sent to the LLM)")
        st.text_area("user_prompt", prompt_package.user_prompt, height=350, label_visibility="collapsed")

        st.download_button(
            "⬇ Download full prompt as .txt",
            data=prompt_package.full_text,
            file_name="prompt.txt",
            mime="text/plain",
        )

    with tab_answer:
        st.caption(f"Model: `{model_choice}`  ·  Temperature: {temperature}  ·  Max tokens: {max_tokens}")

        generate = st.button("🚀 Generate answer via OpenRouter")
        ...
        if "answer" in st.session_state:
            ...
            st.info("Click **Generate answer via OpenRouter** to send the prompt above to the LLM.")
